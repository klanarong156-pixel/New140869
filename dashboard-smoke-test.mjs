...
const checks = [
  ...
  ['MQTT reconnects after browser online, page visibility, and worker loss', /scheduleReconnect\(/.test(handler) && /addEventListener\('online'/.test(handler) && /visibilitychange/.test(handler) && /worker-crashed/.test(handler) && /client-closed/.test(handler) && /mqtt-shared-worker\.js\?v=2/.test(handler)],
