...
const checks = [
  ...
  ['MQTT reconnect has one owner without page-level duplicate timers', /scheduleReconnect/.test(worker) && /reconnectPeriod: 0/.test(worker) && /SharedWorker owns the reconnect timer/.test(handler) && !/setTimeout\(\(\) => \{[\s\S]*this\.connect\(\)/.test(handler)],
  ...
  ['Dashboard quick-control section is removed', !/id="control"/.test(index) && !/data-relay-toggle/.test(index) && !/data-emergency-stop/.test(index) && !/data-emergency-reset/.test(index)],
  ['Dashboard has no orphaned relay/timer handlers', !/data-relay-toggle|data-timer-start|data-timer-cancel|data-timer-preset|renderRelayTimer|commandRelayTimer|commandRelay\(/.test(app)],
  ['Dashboard has no orphaned emergency handlers', !/function emergencyStop|function resetEmergencyStop|data-emergency-stop|data-emergency-reset/.test(read('farm-tools.js') + index)],
  ['Unlimited timer remains supported by MQTT and firmware', /relayTimerSet: relay =>/.test(cfg) && /UNLIMITED/.test(firmware)],
  ...
  ['Homepage clock uses NTP heartbeat time directly', /device:data/.test(clock) && /device\.time/.test(clock) && /timeSource/.test(clock) && /data-farm-time/.test(index) && /data-farm-clock-source/.test(index)],
  ...
  ['Firmware validates NTP clock and monotonic fallback', /bool readCurrentTime/.test(firmware) && /currentClockEpoch/.test(firmware) && /MIN_VALID_EPOCH/.test(firmware) && /millis\(\) - lastNtpSyncMillis/.test(firmware)],
  ['Firmware re-anchors NTP after Wi-Fi reconnect', /lastNtpSyncMillis = 0/.test(firmware) && /void syncNtpTime/.test(firmware) && /ntp\.forceUpdate\(\)/.test(firmware)],
  ...
  ['Firmware retains latest NTP heartbeat', /device\/status/.test(firmware) && /mqtt\.publish\(MQTT_BASE "\/device\/status", out, true\)/.test(firmware) && /timeSource/.test(firmware)],
  ...
  ['Firmware schedules use validated NTP with offline fallback', /scheduleClockMinutes/.test(firmware) && /readCurrentTime\(now\)/.test(firmware) && /applyAutoState\(scheduleMinutes\)/.test(firmware)],
  ...
  ['Timer expiry resolves against NTP schedule before OFF', /scheduleKeepsOn/.test(firmware) && /readCurrentTime\(timerNow\)/.test(firmware) && /if \(!scheduleKeepsOn\) relaySetRaw\(i, false\)/.test(firmware)],
  ...
  ['Board reference matches firmware and settings link', /SmartFarm_V6\.0|V7\.1\.1-NTP-FALLBACK/.test(boardReference) && /DHT11_DATA|DHT11 data/.test(boardReference) && /RELAY_PUMP|ปั้มน้ำ/.test(boardReference) && /BOARD_REFERENCE\.md/.test(settings)],
  ...
  ['Emergency stop remains implemented in firmware', /EMERGENCY_STOP/.test(firmware) && /engageEmergencyStop/.test(firmware) && /relaySetRaw\(i, false\)/.test(firmware)],
  ['Emergency latch status handling remains available in application logic', /emergency:status/.test(app) && /renderEmergency/.test(app)],
